            ...
            <details>
                ...
                <div class="collapse-title grid grid-cols-1 gap-x-3 gap-y-6 place-items-center sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
                    <div class="pl-8" v-for="room in roomsOnFloor(floor)" :key="room.ROOM_ID">
                        <!-- Utilizza una classe condizionale per cambiare il colore della card -->
                        <div :class="['card', 'w-72', 'h-16', 'shadow-xl', 'border-2', 'flex', 'flex-col', 'justify-center', 'items-center', getCardColor(room.PuntoDiRaccolta)]">
                            <div class="flex space-x-4 justify-center items-center">
                                <div class="text-xl">{{ room.Longname }}</div>
                                <div class="flex">
...
export default {
    data() {
        return {
            ...
            colors: ['bg-red-600', 'bg-indigo-600', 'bg-yellow-400', 'bg-green-800', 'bg-teal-400', 'bg-purple-800'] // Definisci i sei colori
        }
    },
    mounted() {
